...
api_key = "182d928de75e016dca3abe96e652cb6b6266e1cb"  # 填写你自己的api_key
minsim = '50!'  # 相似度下限，低于下限不显示结果
thumbSize = (250, 250)
    ...
    while counter <= MAX_NUM:
        ...
        if status_code != 200:
            ...
            if int(results['header']['user_id']) > 0:
                print('搜索限制 30s|24h: ' + str(results['header']['short_remaining']) + '|' + str(
    ...
    if processResults:
        ...
        if int(results['header']['results_returned']) > 0:
            ...
            if float(results['results'][0]['header']['similarity']) > float(results['header']['minimum_similarity']):
                ...
                await bot.send(ev, '相似度' + str(
                    results['results'][0]['header']['similarity']) + '过低,可能：确实找不到此图/图为原图的局部图/图清晰度太低/搜索引擎尚未同步新图')
